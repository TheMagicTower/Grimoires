...
/**
 * Required fields for MCP server configuration
 */
 ...
 * @returns {{ valid: boolean, errors: string[], warnings: string[], servers: string[] }}
 */
 ...
 * @returns {{ valid: boolean, files: Object[] }}
 */
...
Usage: mcp-validator.js [path] [options]

Arguments:
  path           File or directory to validate (default: ./mcp)
...
Examples:
  mcp-validator.js
  mcp-validator.js ./mcp/codex.json
  mcp-validator.js ./mcp --json
`);
    process.exit(0);
  }

  const targetPath = args.find(a => !a.startsWith('--')) || './mcp';
  ...
  if (fs.existsSync(targetPath) && fs.statSync(targetPath).isDirectory()) {
    ...
    const fileResult = validateFile(targetPath);
    fileResult.file = path.basename(targetPath);
    results = {
